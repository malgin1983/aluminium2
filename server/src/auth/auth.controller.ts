...
export class AuthController {
    ...
    async register(@Req() req, @Body() body, @Res() res) {
        try {
            ...
            if (user) {
                return res.status(HttpStatus.CONFLICT).json({ message: 'username conflict' });
            } else {
    ...
    async me(@Req() req, @Res() res) {
        try {
            ...
            if (!result) {
                return res.status(HttpStatus.NOT_FOUND).json({ message: 'token not found' });
            }
    ...
    async authorization(req): Promise<any> {
        ...
        return jwt.verify(token, 'nzblsjdbflahbdslfjhbalsdjbflajhdsblfhasbdflhj', (err, decoded) => decoded);
    }
}
